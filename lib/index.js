...
HugoLunr.prototype.readFile = function(filePath){
	...
	if (ext == '.md'){
		var origContent = meta.content;

		// TODO check for existance!
		// Add labor
		if (typeof meta.data.labor !== 'undefined') {
			origContent = origContent + "\n" + striptags(meta.data.labor);
		}

		// Add timegrapher_old
		if (typeof meta.data.timegrapher_old !== 'undefined') {
			origContent = origContent + "\n" + striptags(meta.data.timegrapher_old.description);
		}

		// Add timegrapher
		if (typeof meta.data.timegrapher !== 'undefined') {
			origContent = origContent + "\n" + striptags(meta.data.timegrapher.description);
		}

		var plainText = removeMd(origContent);

		plainText = plainText.replace(/\{\{.*?\}\}/g, "");
		var abstract = meta.data.description;
	} else {
	...
	// Strip the trailing language code and replace it by a slash
	uri = uri.replace(/\.(de|en)$/g,"/" );

	// Strip the trailing /_index/
	uri = uri.replace(/_index\/$/g, "");

	// Remove multiple newslines from abstract and content
	...
	var item = {'uri' : uri , 'title' : meta.data.title, 'abstract' : abstract, 'content':plainText, 'tags':tags};
	self.list.push(item);
}
